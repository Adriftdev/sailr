...
pub fn generate_deployment_plan(env_name: &str, context: &str) -> Result<DeploymentPlan> {
    ...
    let current_resources = get_current_cluster_resources(context)?;
...
fn get_current_cluster_resources(context: &str) -> Result<HashMap<String, Value>> {
    ...
    // Get all deployments
    if let Ok(deployments) = get_cluster_resources(context, "deployments") {
    ...
    // Get all services
    if let Ok(services) = get_cluster_resources(context, "services") {
    ...
    // Get all configmaps
    if let Ok(configmaps) = get_cluster_resources(context, "configmaps") {
    ...
    // Get all secrets
    if let Ok(secrets) = get_cluster_resources(context, "secrets") {
    ...
    // Get all ingresses
    if let Ok(ingresses) = get_cluster_resources(context, "ingresses") {
    ...
    // Get all HPAs
    if let Ok(hpas) = get_cluster_resources(context, "hpa") {
        for hpa in hpas {
...
fn get_cluster_resources(context: &str, resource_type: &str) -> Result<Vec<Value>> {
    let output = Command::new("kubectl")
        .args(&["--context", context, "get", resource_type, "-o", "json"])
        .output()
        .map_err(|e| anyhow::anyhow!("Failed to execute kubectl: {}", e))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(anyhow::anyhow!("kubectl command failed: {}", stderr));
    }

    let stdout = String::from_utf8_lossy(&output.stdout);
    let json_value: Value = serde_json::from_str(&stdout)
        .map_err(|e| anyhow::anyhow!("Failed to parse kubectl output: {}", e))?;

    if let Some(items) = json_value.get("items").and_then(|i| i.as_sequence()) {
...
fn analyze_manifest_file(
    ...
    for doc in docs {
        if let Some(obj) = doc.as_mapping() {
            ...
            let metadata = obj.get("metadata").and_then(|m| m.as_mapping());
...
fn generate_resource_details(
    ...
    match action {
        ChangeAction::Create => {
            if let Some(spec) = desired_resource.get("spec").and_then(|s| s.as_mapping()) {
                ...
                if let Some(template) = spec
                    ...
                    .and_then(|c| c.as_sequence())
                    .and_then(|containers| containers.first())
                    .and_then(|container| container.as_mapping())
                {
        ...
        ChangeAction::Update => {
            if let Some(current) = current_resource {
                if let Some(spec) = desired_resource.get("spec").and_then(|s| s.as_mapping()) {
                    if let Some(current_spec) = current.get("spec").and_then(|s| s.as_mapping()) {
                        if let Some(replicas) = spec.get("replicas").and_then(|r| r.as_u64()) {
                            ...
                            .and_then(|container| container.as_mapping())
                        {
                            ...
                            if let Some(env_vars) =
                                template.get("env").and_then(|e| e.as_sequence())
                            {
                            ...
                            // Compare volumes
                            if let Some(volumes) =
                                template.get("volumes").and_then(|v| v.as_sequence())
                            {
                            ...
                            if let Some(resources) =
                                template.get("resources").and_then(|r| r.as_mapping())
                            {
